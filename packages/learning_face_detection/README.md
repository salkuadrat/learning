...
You can use widget `InputCameraView` from `learning_input_image` as default implementation for processing image (or image stream) from camera / storage into `InputImage` format. Feel free to learn the inside of `InputCameraView` code if you want to create a custom implementation for `InputImage`.
